...
print("""\n  I am not responsible for your work and if you get some error while
...
def scan(links):
    for link in links:  # fetches one link from the links list
        ...
        if http == 200:  # if its 200 the url points to valid resource i.e. admin panel
            ...
            print(
                '  \033[1;32m[+]\033[0m Potential EAR vulnerability found : ' + link)
        else:
            print('  \033[1;31m[-]\033[1;m %s' % link)
...
def get_paths(type):
    try:
        ...
        pathdone = os.popen("pwd").read()
        pathdone = pathdone[0:-1] + "/paths.txt"
        with open(pathdone, 'r') as wordlist:
